<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
<title>Insert title here</title>
</head>
<body>
	<section class="breadcrumb-section set-bg" data-setbg="../img/breadcrumb.jpg">
		<div class="container">
			<div class="row">
				<div class="col-lg-12 text-center">
					<div class="breadcrumb__text">
						<h2>글쓰기</h2>
						<div class="breadcrumb__option">
							<span>목록</span>
						</div>
					</div>
				</div>
			</div>
		</div>
	</section>
	<section class="product-details spad" id="board_list">
		<div class="container">
			<div class="row">
				<table class="table">
					<tbody>
						<tr>
							<td width="10%" class="text-center">이름</td>
							<td width="90%" class="text-left">
								<input type="text" ref="nameRef" v-model="name" size="20">
							</td>
						</tr>
						<tr>
							<td width="10%" class="text-center">제목</td>
							<td width="90%" class="text-left">
								<input type="text" ref="subRef" v-model="subject" size="60">
							</td>
						</tr>
						<tr>
							<td width="10%" class="text-center">내용</td>
							<td width="90%" class="text-left">
								<textarea rows="10" cols="60" ref="contRef" v-model="content"></textarea>
							</td>
						</tr>
						<tr>
							<td width="10%" class="text-center">비밀번호</td>
							<td width="90%" class="text-left">
								<input type="password" ref="pwdRef" v-model="pwd" size="10">
							</td>
						</tr>
						<tr>
							<td colspan="2" class="text-center">
								<button class="btn-sm btn-success">글쓰기</button>
								<button class="btn-sm btn-info" onclick="javascript:history.back()">취소</button>
							</td>
						</tr>
					</tbody>
				</table>
			</div>
		</div>
	</section>
</body>
</html>