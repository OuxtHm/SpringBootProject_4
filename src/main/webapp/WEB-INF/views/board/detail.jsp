            	...
            	<table class="table">
            		<tbody>
            			<tr>
            				...
            				<td width="30%" class="text-cneter">{{store.vo.no}}</td>
            				<th width="20%" class="text-center success">작성일</th>
            				<td width="30%" class="text-cneter">{{store.vo.dbday}}</td>
            			</tr>
            			<tr>
            				<th width="20%" class="text-center success">이름</th>
            				<td width="30%" class="text-cneter">{{store.vo.name}}</td>
            				<th width="20%" class="text-center success">조회수</th>
            				<td width="30%" class="text-cneter">{{store.vo.hit}}</td>
            			</tr>
            			<tr>
            				<th width="20%" class="text-center success">제목</th>
            				<td colspan="3" class="text-cneter">{{store.vo.subject}}</td>
            			</tr>
            			<tr>
            				<td colspan="4" class="text-left" valign="top" heifht="200">
            					<pre style="white-space: prewrap; border: none; background-color: white">{{store.vo.content}}</pre>
            				</td>
            			</tr>
            			<tr>
            				<td colspan="4" class="text-right">
            					<a :href="'/board/update?no='+store.vo.no" class="btn btn-xs btn-warning">수정</a>
            					<a :href="'/board/delete?no='+store.vo.no" class="btn btn-xs btn-warning">삭제</a>
